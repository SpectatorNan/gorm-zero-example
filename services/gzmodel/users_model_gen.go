...
func (m *defaultUsersModel) FindOne(ctx context.Context, id int64) (*Users, error) {
	gormzeroUsersIdKey := fmt.Sprintf("%s%v", cacheGormzeroUsersIdPrefix, id)
	var resp Users
	err := m.QueryRowCtx(ctx, &resp, gormzeroUsersIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultUsersModel) Insert(ctx context.Context, data *Users) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Account, data.NickName, data.Password, data.DeletedAt)
	}, gormzeroUsersIdKey)
	return ret, err
}
...
func (m *defaultUsersModel) Update(ctx context.Context, data *Users) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Account, data.NickName, data.Password, data.DeletedAt, data.Id)
	}, gormzeroUsersIdKey)
	return err
}
